...
export class EditProfilePage implements OnInit {
  ...
  update() {
    ...
    this.util.show();
    this.api.updateProfile(localStorage.getItem('uid2'), param).then((data: any) => {
      console.log(data);
      this.util.hide();
  ...
  opemCamera(type) {
    ...
    this.camera.getPicture(options).then((imageData) => {
      ...
      this.util.show();
      const id = localStorage.getItem('uid2') + '/' + this.util.makeid(10);
      firebase.storage().ref().child(localStorage.getItem('uid2')).child(btoa(id) + '.jpg')
