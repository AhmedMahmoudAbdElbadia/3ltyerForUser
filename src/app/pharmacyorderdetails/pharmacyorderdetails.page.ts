...
@Component({
  selector: 'app-pharmacyorderdetails',
  templateUrl: './pharmacyorderdetails.page.html',
  styleUrls: ['./pharmacyorderdetails.page.scss'],
})
export class PharmacyorderdetailsPage implements OnInit {
  id: any;
  grandTotal: any;
  orders: any[] = [];
  serviceTax: any;
  ...
  getOrder() {
    ...
    this.api.getPharmacyOrderById(this.id).then((data) => {
      this.loaded = true;
 
      console.log(typeof data,"Data Here");
      if (data) {
      //let data2=  data.toString();
      this.orderData=data;
     //   this.grandTotal = data.grandTotal;
      // this.orders = JSON.parse(data.order);
        console.log(this.orderData.drugname,"orders")
        // this.serviceTax = data.serviceTax;
        this.drugname=this.orderData.drugname;
